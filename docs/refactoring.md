...
- ただしこれは"プロセス内メモリ"のキャッシュであり、Serverless/Edge 環境やプロセス再起動時には保持されず、常に効果が得られるわけではない点を明記する。
- あわせて、長時間稼働時のメモリ増加や、データを動的に更新する要件が生じた場合のキャッシュ無効化・再読込戦略も運用上の注意点として整理する。
- 永続性や再検証が必要な場合は、Next.js 側の `unstable_cache` や `revalidateTag` などの仕組みも候補として併記し、用途に応じて使い分ける方針を示す。
